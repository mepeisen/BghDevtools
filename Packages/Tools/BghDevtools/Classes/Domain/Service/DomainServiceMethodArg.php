...
/**
 * Service method argument definition
 *
 * @license http://www.gnu.org/licenses/gpl.html GNU General Public License, version 3 or later
 */
class DomainServiceMethodArg extends \F3\BghDevtools\Domain\NamedElement
{
	...
	{
		return 'Service-Method-Argument-Definition';
	}
	...
	protected function validate()
	{
		...
		if ($this->type === false)
		{
			throw new \F3\BghDevtools\Domain\Service\Exception("Error reading service method argument '".$this->getName()."'. Missing type", 1286874209);
		}
	
		if ($this->translate && $this->shadowed)
		{
			throw new \F3\BghDevtools\Domain\Service\Exception("Error reading service method argument '".$this->getName()."'. Translate and shadowed cannot be used together", 1286874210);
		}
	
		if ($this->shadowed && $this->value === false)
		{
			throw new \F3\BghDevtools\Domain\Service\Exception("Error reading service method argument '".$this->getName()."'. Missing value for shadowed argument", 1286874211);
		}
		
		if (!$this->shadowed && $this->value !== false)
		{
			throw new \F3\BghDevtools\Domain\Service\Exception("Error reading service method argument '".$this->getName()."'. Value on regular argument", 1286874212);
		}
	}
