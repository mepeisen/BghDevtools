 ...
 */
class DomainService extends \F3\BghDevtools\Domain\NamedElement
{
	...
	/**
	 * @var array(string=>\F3\BghDevtools\Domain\Service\DomainServiceMethod)
	 */
	 ...
	 * Returns the methods
	 * 
	 * @return array(string=>\F3\BghDevtools\Domain\Service\DomainServiceMethod)
	 */
	...
	protected function applyChild(\SimpleXmlElement $element)
	{
		...
		if ($element->getName() == 'method')
		{
			try
			{
				$method = $this->objectManager->create('F3\BghDevtools\Domain\Service\DomainServiceMethod');
				$method->parse($element);
				if (isset($this->methods[$method->getName()]))
				{
					throw new \F3\BghDevtools\Domain\Service\Exception("Duplicate service method for service '".$this->getName()."': '".$method->getName()."'", 1286872285);
				}
			...
			{
				throw new \F3\BghDevtools\Domain\Service\Exception("Error reading service '".$this->getName()."'. Nested code: ".$e->getCode()." / Nested message: ".$e->getMessage(), 1286872288);
			}
		}
